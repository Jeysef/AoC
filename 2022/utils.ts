...
export const readFile = (name: string) => fs.readFileSync(path.join(__filename, `../${name}.txt`), 'utf8').trim();
export const readFileSplitted = (name: string) => fs.readFileSync(path.join(__filename, `../${name}.txt`), 'utf8').trim().split('\r\n');
